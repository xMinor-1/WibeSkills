...
## Todo-лист проекта в GitHub Issues (постоянное правило)

Файловый бэклог `stories.md` держит **текущий эпик**: Story и Task внутри pipeline. Всё, что
не относится к текущему эпику — идея, замечание, поломка, «надо будет сделать» из разговора —
попадает в **GitHub Issues репозитория проекта** и оттуда на доску Taskboard в bb. Это
правило действует во всех скиллах, а не только в `todo`.

| Где | Что живёт |
|---|---|
| `stories.md` | Story и Task текущего эпика, статусы внутри pipeline |
| GitHub Issues | сквозной todo-лист проекта: идеи, замечания, баги, задачи вне эпика |
| Доска Taskboard | отображение GitHub Issues: Todo → In Progress → Done |

Правила:

- Задачу заводить **в момент договорённости**, а не в конце сессии: `gh issue create --repo <owner/repo>`.
  Заголовок — глагол + результат; тело — только если добавляет смысл.
- Пачку (больше одной) сначала показать списком и получить подтверждение; перед этим проверить
  `gh issue list --state open` на дубли.
- После любых изменений задач прогнать `bb github sync` и `bb taskboard refresh github`,
  иначе доска покажет старое.
- Метки: стадия работы — `coding` / `review` / `qa` / `blocked` (любая из них означает колонку
  In Progress), приоритет — `now` / `next` / `later` / `idea`, поломка — `bug`.
- Story и Task из `stories.md` в Issues дублировать не надо — для зеркала бэклога есть `tracker-sync`.
