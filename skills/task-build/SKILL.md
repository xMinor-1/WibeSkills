...
## Две роли: contract и fill (если `EXECUTOR` ≠ `inherit`)

Код пишет не тот, кто проектировал → Task проходит через две стадии вместо одной.
Протокол — `../_shared/executor-protocol.md`, пакет — `DOCS_ROOT/<epic-slug>/tasks/<task-slug>.md`.
При `EXECUTOR: inherit` (дефолт) обе роли делает один агент — работай по шагам ниже как обычно.

**Режим contract** (топ-модель, всегда у нас):
1. Открой заготовку пакета от `agile-coach`; нет заготовки — создай по шаблону `../_shared/templates/task-packet.md`.
2. Заполни «Контракт интерфейсов»: конкретные сигнатуры, типы, порты, коды ошибок — то, на что
   будут опираться зависимые Task. Существующий код, который надо переиспользовать, — таблицей с путями.
3. Напиши **красные тесты** на AC этой Task и edge cases. Прогони: они обязаны **падать** сейчас.
   Тест, зеленеющий на пустой реализации, бесполезен — переписывай.
4. Коммит: контракт (типы/порты/сигнатуры со стабами) + падающие тесты, отдельным коммитом.
5. Пройди чек-лист в конце шаблона. Верни `status: ok` и путь к пакету.

Не реализуй тела в этом режиме — иначе исполнителю нечего делать, а замер моделей теряет смысл.

**Режим fill** (исполнитель из `EXECUTOR` — младшая модель или внешний CLI):
Вход — только пакет и репозиторий. PRD, tech-spec и переписка не нужны и не даются.
Задача: сделать красные тесты зелёными, не меняя их текст и не выходя за «Разрешено править».
Дальше — `VERIFY_CMD`, лимит авто-fix 3, ответ строго в JSON из пакета.
...
Работаешь в режиме **contract/fill** — тесты уже написаны на стадии contract: в режиме contract
это и есть твой выход, в режиме fill их **нельзя менять**, только делать зелёными.

## Шаг 6: Локальная проверка

Есть `VERIFY_CMD` (project-config) и пакет Task — запусти его: точечные тесты + typecheck +
проверка, что diff не вышел за «Разрешено править». Быстрее полного прогона и даёт машинный вердикт.
Нет `VERIFY_CMD` — `LOCAL_CHECK` как раньше.
