...
- [ ] `_shared/project-config.md`: поля `NMT_CANON_ROOT` (дефолт — `canon/Next-Move-Theory-Canon/` относительно репо) и опциональный `COURSE_NOTES_ROOT` (приватный слой курса, вне репо)
- [ ] `_shared/nmt-canon-routing.md` — таблица «задача → файл канона» (адаптация роутинга из `canon/CLAUDE.md`)
...
- [ ] **Портировать `ask-nmt`** — разговорный советник по методологии: lazy-роутинг по канону, 5 режимов (объясни / диагностируй / прожми / примени / научи), хендоффы в наши скиллы-продюсеры. Русский, без файлов-артефактов по умолчанию.
